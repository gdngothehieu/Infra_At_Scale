...
resource "aws_lambda_function" "lambda_udacity" {
  ...
  environment {
		variables = {
			greeting = "Hi from Design, Provision and Monitor AWS Infrastructure at Scale Task 5"
		}
	}
}

resource "aws_cloudwatch_event_rule" "rule_lambda_every_one_minute" {
  name = "rule_lambda_every_one_minute"
  depends_on = [
...
resource "aws_cloudwatch_event_target" "lambda_udacity" {
  ...
  rule = "${aws_cloudwatch_event_rule.rule_lambda_every_one_minute.name}"
  arn = "${aws_lambda_function.lambda_udacity.arn}"
}

resource "aws_lambda_permission" "permission_lambda_every_one_minute" {
  statement_id = "AllowExecutionFromCloudWatch"
  action = "lambda:InvokeFunction"
  function_name = "${aws_lambda_function.lambda_udacity.function_name}"
  principal = "events.amazonaws.com"
  source_arn = "${aws_cloudwatch_event_rule.rule_lambda_every_one_minute.arn}"
}
